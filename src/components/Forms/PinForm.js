...
export default class PinForm extends Component {
  ...
  handleSubmit = (e) => {
    ...
    if (this.state.firebaseKey === '') {
      ...
      pinData.createPin(newPin).then((response) => {
        const pinBoard = {
      ...
      pinData.getPinBoardToDelete(this.state.firebaseKey);
      const updatedPin = {
        name: this.state.name,
        description: this.state.description,
        imageUrl: this.state.imageUrl,
      ...
      pinData.updatePin(updatedPin).then(() => {
  ...
  getBoards = (uid) => {
    console.warn("I'm calling getBoards");
    return boardData.getAllUserBoards(uid).then((response) => response);
  }
